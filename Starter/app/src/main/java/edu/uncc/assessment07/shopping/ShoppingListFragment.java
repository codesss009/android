...
mShoppingList.getDocId();
        ...
        db.collection("Shopping Lists").document(mShoppingList.getDocId()).collection("Shopping List Items").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
        ...
        class ShoppingListViewHolder extends RecyclerView.ViewHolder{
            ...
            public void setupUI(ShoppingListItem shoppingListItem){
                ...
                mBinding.imageViewAdd.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        FirebaseFirestore db = FirebaseFirestore.getInstance();
                        mShoppingListItem.quantity += 1;
                        db.collection("Shopping Lists").document(mShoppingList.docId).collection("Shopping List Items").document(mShoppingListItem.docId).set(mShoppingListItem).addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                ...
                mBinding.imageViewMinus.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (mShoppingListItem.quantity == 1){
                            db.collection("Shopping Lists").document(mShoppingList.docId).collection("Shopping List Items").document(mShoppingListItem.docId).delete();
                        }
                            else{
                            mShoppingListItem.quantity -= 1;
                            db.collection("Shopping Lists").document(mShoppingList.docId).collection("Shopping List Items").document(mShoppingListItem.docId).set(mShoppingListItem).addOnCompleteListener(new OnCompleteListener<Void>() {
